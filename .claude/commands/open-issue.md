...
1. **Determine the issue type and gather its substance** from the conversation and the argument:
    ...
    - If the available context is too thin to write a specific, accurate issue, stop and ask the user for the missing details (for a bug, the symptom and where it occurs, and for a feature or task, the problem and the proposed approach). Never fabricate an issue.
