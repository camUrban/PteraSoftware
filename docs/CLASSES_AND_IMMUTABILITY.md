...
| Attribute                 | Type                         | Notes                                   |
|---------------------------|------------------------------|-----------------------------------------|
| `forces_W`                | `list[np.ndarray]`           | Forces at each time step                |
| `forceCoefficients_W`     | `list[np.ndarray]`           | Force coefficients at each time step    |
| `moments_W_Cg`            | `list[np.ndarray]`           | Moments at each time step               |
| `momentCoefficients_W_Cg` | `list[np.ndarray]`           | Moment coefficients at each time step   |
| `coupled_steady_problems` | `list[CoupledSteadyProblem]` | Solver appends each time step's problem |
...
| Property     | Depends On           | Notes                       |
|--------------|----------------------|-----------------------------|
| `max_period` | `airplane_movement`  | Delegates to child movement |
| `static`     | `max_period`         | True if max_period is 0     |
...
| Attribute                 | Type                    | Notes                                   |
|---------------------------|-------------------------|-----------------------------------------|
| `airplane`                | `Airplane`              | Single Airplane (not a list)            |
| `coupled_operating_point` | `CoupledOperatingPoint` | Operating conditions for this time step |

**Note**: During `__init__`, the Airplane's Panel GP1_CgP1 coordinates are populated (same as SteadyProblem). The Airplane's `Cg_GP1_CgP1` must be zeros since CoupledSteadyProblems support only a single Airplane.

## CoupledOperatingPoint Class (`operating_point.py`)

This class is a subclass of OperatingPoint. It inherits all of OperatingPoint's immutable attributes, cached derived properties, and behavior without modification. It adds the following additional attributes and properties for coupled (free flight) simulations.
...
| Attribute              | Type         | Notes                                          |
|------------------------|--------------|------------------------------------------------|
| `omegas_BP1__E`        | `np.ndarray` | Angular speed of body axes (Earth frame)       |
| `angles_E_to_BP1_izyx` | `np.ndarray` | Angles from Earth to body axes (intrinsic zyx) |
| `g_E`                  | `np.ndarray` | Gravitational acceleration in Earth axes       |

#### Derived (not cached, computed fresh each call)

| Property                   | Depends On                                               | Notes                         |
|----------------------------|----------------------------------------------------------|-------------------------------|
| `T_pas_E_CgP1_to_BP1_CgP1` | `angles_E_to_BP1_izyx`                                   | Earth to body transformation  |
| `T_pas_BP1_CgP1_to_E_CgP1` | `T_pas_E_CgP1_to_BP1_CgP1`                               | Body to Earth transformation  |
| `T_pas_E_CgP1_to_GP1_CgP1` | `T_pas_E_CgP1_to_BP1_CgP1`, `T_pas_BP1_CgP1_to_GP1_CgP1` | Earth to geometry (composed)  |
| `T_pas_GP1_CgP1_to_E_CgP1` | `T_pas_E_CgP1_to_GP1_CgP1`                               | Geometry to Earth (inverted)  |
| `T_pas_E_CgP1_to_W_CgP1`   | `T_pas_E_CgP1_to_BP1_CgP1`, `T_pas_BP1_CgP1_to_W_CgP1`   | Earth to wind (composed)      |
| `T_pas_W_CgP1_to_E_CgP1`   | `T_pas_E_CgP1_to_W_CgP1`                                 | Wind to Earth (inverted)      |
| `vCg_E__E`                 | `vCg__E`, `T_pas_W_CgP1_to_E_CgP1`                       | Velocity vector in Earth axes |
