...
- Define module-level constants after the imports and before the first function or class. There are two exceptions. First, a constant whose value calls a function defined in the same file goes after that function (e.g., `SEQUENTIAL_COLOR_MAP` in `_colormaps.py`). Second, in a script that runs from top to bottom, such as `validation/validation_study.py`, a constant may sit just above the step that uses it.
