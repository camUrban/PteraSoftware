...
spring constant (K) and flapping frequency, then plots the resulting trailing-edge
amplitude and wingtip deformation angle amplitude against frequency, with one
colored line per K value, to validate against the kinematics figure of
trailing-edge amplitude versus reduced frequency."""
...
# Default values used for the parameters that are not being swept
DEFAULT_B = 1000.0
DEFAULT_DENSITY = 6

# The spring constants (K) and flapping frequencies to sweep. Every combination of
# the two is run.
K_VALUES: list[float] = [0, 20000, 40000.0, 80000.0, 1600000.0]
FREQUENCY_VALUES_HZ: list[float] = [1.0, 3.0, 5.0, 6.0, 8.0, 10.0]
# K_VALUES: list[float] = [100.0, 1000.0, 10000.0]
# FREQUENCY_VALUES_HZ: list[float] = [2.0, 2.5, 3.0]

# A very large spring constant representing the rigid-wing (K -> infinity) limit.
# Its trailing-edge motion is purely kinematic (no elastic deformation), so dividing
# every other run's trailing-edge amplitude by this reference's, at the same
# frequency, turns the y-axis into a gain that is exactly 1.0 for a rigid wing,
# matching the validation figure's flat black K = infinity curve.
K_RIGID = 1.0e8

# Time steps per flapping cycle, held fixed across the frequency sweep. DELTA_TIME is
...
def run_aeroelastic(
    ...
    cross_section_chords = [1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75]
    wing_cross_sections = []
    ...
    example_operating_point = ps.operating_point.OperatingPoint(
        rho=0.1, vCg__E=10.0, alpha=0.0, beta=0.0, externalFX_W=0.0, nu=15.06e-6
    )
...
# For each frequency, first run a rigid-wing (K -> infinity) reference case to get
# the trailing-edge amplitude with no elastic deformation. If the prescribed motion
# ever drives a WingCrossSection's deformation angle past the geometric +/-90 degree
# limit, the run raises a ValueError; that frequency is skipped entirely (for every
# K) since there is no reference to normalize against.
reference_te_amplitude_by_frequency = {}
for frequency_hz in FREQUENCY_VALUES_HZ:
    delta_time = (1.0 / frequency_hz) / STEPS_PER_CYCLE

    print(f"Running rigid reference at frequency={frequency_hz} Hz...")
    try:
        _, reference_problem = run_aeroelastic(
            spring_constant=K_RIGID,
        ...
        print(f"  Skipping frequency={frequency_hz} Hz: {error}")
        continue
    reference_te_amplitude_by_frequency[frequency_hz] = steady_state_half_range(
        trailing_edge_z_history(reference_problem), DISCARD_STEPS
    )
    print(
        f"  Rigid TE amplitude={reference_te_amplitude_by_frequency[frequency_hz]:.4f} m"
    )

# Run every (K, frequency) combination and collect the trailing-edge amplitude gain
# (relative to the rigid-wing reference at the same frequency) and the wingtip
...
# limit raises a ValueError; that single (K, frequency) data point is skipped rather
# than aborting the whole sweep. Each K's results and the frequencies they correspond
# to are tracked together so the plots only connect points that actually ran.
frequencies_used = {k: [] for k in K_VALUES}
te_amplitude_gain = {k: [] for k in K_VALUES}
angle_amplitudes_deg = {k: [] for k in K_VALUES}
wingtip_curve_index = None
for k in K_VALUES:
    for frequency_hz in FREQUENCY_VALUES_HZ:
        if frequency_hz not in reference_te_amplitude_by_frequency:
            print(
                f"Skipping K={k}, frequency={frequency_hz} Hz: no rigid reference "
                "available."
            )
            continue
        delta_time = (1.0 / frequency_hz) / STEPS_PER_CYCLE

        print(f"Running K={k}, frequency={frequency_hz} Hz...")
        try:
            ...
            print(f"  Skipping K={k}, frequency={frequency_hz} Hz: {error}")
            continue
        ...
        gain = te_amplitude / reference_te_amplitude_by_frequency[frequency_hz]
        ...
        frequencies_used[k].append(frequency_hz)
...
colors = plt.get_cmap("viridis")(np.linspace(0.0, 1.0, len(K_VALUES)))

# Trailing-edge amplitude gain versus flapping frequency, one colored line per K,
# plus the rigid-wing reference (gain = 1.0 by construction) for comparison.
...
rigid_frequencies = sorted(reference_te_amplitude_by_frequency)
plt.plot(
    rigid_frequencies,
    np.ones(len(rigid_frequencies)),
    color="black",
    linestyle="--",
    label="K=inf (rigid)",
)
...
plt.title("Trailing-Edge Amplitude Gain vs. Flapping Frequency")
plt.legend()
plt.grid(True)
plt.savefig("Trailing_Edge_Amplitude_Gain_vs_Frequency.png")
plt.show()

# Wingtip deformation angle amplitude versus flapping frequency, one colored line
# per K.
plt.figure(figsize=(10, 6), dpi=200)
for color, k in zip(colors, K_VALUES):
    plt.plot(
...
plt.title("Deformation Angle Amplitude vs. Flapping Frequency")
plt.legend()
plt.grid(True)
plt.savefig(f"Deformation_Angle_Amplitude_Curve_{wingtip_curve_index}_vs_Frequency.png")
plt.show()
