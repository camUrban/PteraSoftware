...
# at the start of free flight. The glider flies an unpowered glide: no external thrust
# is applied, and thrust would instead be modeled with FreeFlightUnsteadyProblem's
# external_loads_fn. Standard gravity is set explicitly via g_E (the default is no
# gravitational field), while the zero initial body rates (omegas_BP1__E) are left at
# their default.
