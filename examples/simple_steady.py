        ...
        ps.geometry.Wing(
            wing_cross_sections=[
                ...
                ps.geometry.WingCrossSection(
                    ...
                    z_le=z,
                    chord=2.0,
                    # unit_normal_vector=np.array([0.0, np.cos(theta), np.sin(theta)]),
                    twist=0.0,
                    control_surface_type="symmetric",
                    control_surface_hinge_point=0.75,
                    control_surface_deflection=0.0,
...
simple_operating_point = ps.operating_point.OperatingPoint(
    density=1.225, velocity=10.0, alpha=5.0, beta=0.0, external_thrust=0.0, nu=1.5e-5
)

simple_static_problem = ps.problems.SteadyProblem(
    airplanes=[simple_airplane], operating_point=simple_operating_point
)

# simple_steady_solver = (
#     ps.steady_ring_vortex_lattice_method.SteadyRingVortexLatticeMethodSolver(
#         steady_problem=simple_static_problem
#     )
# )
simple_steady_solver = (
...
del simple_airplane
del simple_operating_point
del simple_static_problem

simple_steady_solver.run()

ps.output.print_steady_results(steady_solver=simple_steady_solver)

# ps.output.draw(
#     solver=simple_steady_solver,
#     scalar_type="lift",
#     show_streamlines=True,
#     show_wake_vortices=False,
# )

wing = simple_steady_solver.airplanes[0].wings[0]
panels = wing.panels
show_z = True
...
for row_of_panels in panels:
    ...
    for panel in row_of_panels:
        ...
        # this_cd = panel.induced_drag_coefficient
        # this_cl = panel.lift_coefficient
        # this_cy = panel.side_force_coefficient
        # text = "({: .5e}, {: .5e}, {: .5e})"
        # this_text = text.format(
        #     this_cd,
        #     this_cl,
        #     this_cy,
        # )
