...
lattice method solver on a
custom airplane with variable geometry."""

# First, import the software's main package. Note that if you wished to import this
# software into another package, you
# would first install the software by running "pip install pterasoftware" in your
# terminal. Then, at the top of your
...
# class, even most of them have usable
# default values. This is simply for educational purposes, even though it makes the
# code much longer than what it needs
# to be.
example_airplane = ps.geometry.Airplane(
    ...
    # around about which the solver will
    # calculate the moments on the airplane. These three values default to 0.0
    # meters. Note that every input and output
    # of this program is in SI units.
    x_ref=0.0,
    y_ref=0.0,
    z_ref=0.0,
    # Give the reference dimensions of this aircraft. "s_ref" is the reference area
    # in meters squared, "b_ref" is the
    # reference span in meters, and "c_ref" is the reference chord in meters. I set
    # these values to None, which is their
    # default, so that they will be populated by the first wing object's calculated
    # characteristics. Note that the
    # reference area used in this program is the wetted area of the wing's
    # mean-camberline surface.
    s_ref=None,
    b_ref=None,
    ...
    wings=[  # Create the first wing object in this airplane.
        ps.geometry.Wing(  # Give the wing a name, this defaults to "Untitled Wing".
            ...
            # airplane's reference position. These
            # values all default to 0.0 meters.
            x_le=0.0,
            y_le=0.0,
            z_le=0.0,
            # Declare that this wing is symmetric. This means that the geometry will
            # be reflected across the y-z plane.
            # Note that the geometry coordinates are defined as such: If you were
            # riding in the airplane, the positive
            # x direction would point behind you, the positive y direction would
            # point out of your right wing, and the
            # positive z direction would point upwards, out of your chair. These
            # directions form a right-handed
            ...
            # between them. The number of chordwise
            # panels defaults to 8 panels. The spacing defaults to "cosine",
            # which makes the panels relatively finer, in
            # the chordwise direction, near the leading and trailing edges. The other
            # option is "uniform". I set this
            ...
            # output to be sensible, each wing
            ...
            wing_cross_sections=[  # Create a new wing cross section object.
                ps.geometry.WingCrossSection(
                    ...
                    # section relative to the wing's leading
                    # edge. These values all default to 0.0 meters.
                    x_le=0.0,
                    y_le=0.0,
                    z_le=0.0,
                    # Define the twist of the wing cross section in degrees. This is
                    # equivalent to incidence angle of
                    # cross section. The twist is about the leading edge. Note that
                    # the twist is only stable up to 45.0
                    # degrees. Values above that produce unexpected results. This
                    # will be fixed in a future release. The
                    # default value is 0.0 degrees. Positive twist corresponds to
                    # positive rotation about the y axis, as
                    # defined by the right-hand rule.
                    twist=0.0,
                    # Define the type of control surface. The options are "symmetric"
                    # and "asymmetric". This is only
                    # applicable if your wing is also symmetric. If so, symmetric
                    # control surfaces will deflect in the
                    # same direction, like flaps, while asymmetric control surfaces
                    # will deflect in opposite directions,
                    # like ailerons. The default value is "symmetric".
                    control_surface_type="symmetric",
                    # Define the point on the airfoil where the control surface
                    # hinges. This is expressed as a faction
                    # of the chord length, back from the leading edge. The default
                    # value is 0.75.
                    ...
                    # and the spacing between them. The
                    # number of spanwise panels defaults to 8 panels. The spacing
                    # defaults to "cosine", which makes the
                    # panels relatively finer, in the spanwise direction, near the
                    # cross section ends. The other option
                    # is "uniform".
                    ...
                    airfoil=ps.geometry.Airfoil(
                        ...
                        # Airfoil". This name should correspond to a
                        # name in the airfoil directory or a NACA four series
                        # airfoil, unless you are passing in your
                        # own coordinates.
                        name="naca2412",
...
# move in three ways: sweeping,
# pitching, and heaving. Sweeping is defined as the relative rotation of this wing
# cross section's leading edge to its
# preceding wing cross section's leading edge about the airplane's body x axis.
# Pitching is defined as the relative
# rotation of this wing cross section's leading edge to the preceding wing cross
# section's leading edge about the body
# y axis. Heaving is defined as the relative rotation of this wing cross section's
# leading edge to the preceding wing
...
main_wing_root_wing_cross_section_movement = ps.movement.WingCrossSectionMovement(
    ...
    # wing cross section, this must be 0.0
    ...
    # cross section, this must be 0.0
...
# so the movement attributes can be
# excluded, and the default values will suffice.
...
# relative movements, wings' leading edge
...
# cross sections' relative movements,
...
operating_point_movement = ps.movement.OperatingPointMovement(
    ...
    # meters per second, which is the
    # default value.
    ...
    # by default. The options are "sine"
    # and "uniform".
...
movement = ps.movement.Movement(  # Add the airplane movement.
    ...
    # the time in between them, in order
    # to have the wake ring vortices and the bound ring vortices have the same area,
    # and to have the wake ring vortex
    # sheet extend back to be approximately fifteen times the chord length of the
    # main wing.
...
# now contained within the movement
# object.
...
example_solver.run(
    ...
    # "verbose" attribute defaults to
    # true.
    verbose=True,
    # Use a prescribed wake model. This is faster, but may be slightly less accurate.
    # The free wake model is not yet
    # stable.
    prescribed_wake=True,
)

# Call the software's animate function on the solver. This produces a GIF of the wake
# being shed. The GIF is saved in
# the same directory as this script. Press "q", after orienting the view, to begin
# the animation.
...
# output forces and moments with
# respect to time.
