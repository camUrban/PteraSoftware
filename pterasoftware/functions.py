...
This module contains the following functions:
    ...
    generate_rotation_matrix_from_a_to_b: "This function generates a rotation matrix
    that maps from "A" axes to "B" axes using a vector of the angles between them,
    and parameters specifying the type and order of the sequence of rotations.
"""
...
def generate_rotation_matrix_from_a_to_b(angles_A_to_B, intrinsic, order):
    """This function generates a rotation matrix that maps from "A" axes to "B" axes
    using a vector of the angles between them, and parameters specifying the type and
    order of the sequence of rotations.

    This function doesn't check for validity of inputs and doesn't check for gimbal
    lock.

    :param angles_A_to_B: (3,) ndarray of floats
        This is the array of angles that map a vector from "A" to "B" axes. The
        angles are in degrees. The first value is the angle about the axis 1,
        the second value is the angle about axis 2, and the third value is the angle
        about axis 3. They are not necessarily applied in that order.
    :param intrinsic: bool
        If True, each subsequent rotation is applied to newly rotate axes. If False,
        all rotations are performed about the original, non-rotated axes.
    :param order: string of length 3
        This is the string of three integers that represents the rotation order. Each
        character can be 1, 2, or 3. Only Tait-Bryan angles are accepted so no
        character can be a repeated number.
    :return: (3, 3) ndarray of floats
        This is the rotation matrix that maps "A" axes to "B" axes. As we assume all
        vectors are column vectors, it can be applied as follows: F_B = R_A_to_B @
        F_A, where F_B is F in "B" axes, R_A_to_B matrix generated by this function,
        and F_A is F in "A" axes.
    """
    angles_A_to_B = np.array(angles_A_to_B, dtype=float)

    angle_A_to_B_1_rad, angle_A_to_B_2_rad, angle_A_to_B_3_rad = np.radians(
        angles_A_to_B
    )
    ...
    R_A_to_B_1 = np.array(
        [
            [1, 0, 0],
            [0, np.cos(angle_A_to_B_1_rad), -np.sin(angle_A_to_B_1_rad)],
            [0, np.sin(angle_A_to_B_1_rad), np.cos(angle_A_to_B_1_rad)],
        ]
    )
    R_A_to_B_2 = np.array(
        [
            [np.cos(angle_A_to_B_2_rad), 0, np.sin(angle_A_to_B_2_rad)],
            [0, 1, 0],
            [-np.sin(angle_A_to_B_2_rad), 0, np.cos(angle_A_to_B_2_rad)],
        ]
    )
    R_A_to_B_3 = np.array(
        [
            [np.cos(angle_A_to_B_3_rad), -np.sin(angle_A_to_B_3_rad), 0],
            [np.sin(angle_A_to_B_3_rad), np.cos(angle_A_to_B_3_rad), 0],
            [0, 0, 1],
        ]
    )

    R_A_to_B_components = [R_A_to_B_1, R_A_to_B_2, R_A_to_B_3]
    ...
    for order_id in order_ids:
        R_A_to_B = R_A_to_B @ R_A_to_B_components[order_id]
