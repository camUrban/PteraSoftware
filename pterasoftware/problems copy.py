...
#         # Track the primary aeroelastic wing's ODE results so that reflected
#         # counterpart wings (whose SLEP-relative moment indices are unreliable
#         # because slep_point_indices only advances by num_spanwise_panels, not
#         # num_chordwise * num_spanwise) copy the primary wing's deformation
#         # instead of running an independent ODE on incorrect moments.
#         primary_aero_wing_idx: int | None = None
#         primary_step_deformation: np.ndarray | None = None
#         primary_omegas: np.ndarray | None = None
...
#                 if primary_aero_wing_idx is None:
#                     # First aeroelastic wing: run the full ODE.
...
#                     primary_aero_wing_idx = wing_idx
#                     primary_step_deformation = step_deformation
#                     primary_omegas = omegas
#                 else:
#                     # Subsequent aeroelastic wings: copy deformation from the
#                     # primary wing. Running an independent ODE here would use
#                     # incorrect SLEP-relative moments (the slep_point_indices in
#                     # the solver only advance by num_spanwise_panels per WCS
#                     # instead of num_chordwise * num_spanwise, so non-primary
#                     # wings map to front-left points of the wrong wing entirely).
#                     assert primary_step_deformation is not None
#                     assert primary_omegas is not None
#                     inertial_moments = np.zeros(
#                         (num_chordwise_panels, num_spanwise_panels, 3),
#                         dtype=float,
#                     )
#                     step_deformation = primary_step_deformation
#                     omegas = primary_omegas
