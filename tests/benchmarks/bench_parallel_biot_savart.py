...
Compares the parallel Numba kernel against a local serial reference to verify matching
...
def _serial_reference(
    ...
    This is a local copy of the package's original serial Biot-Savart kernel, kept in
    the benchmark so the parallel kernel can be timed against a same algorithm serial
    baseline. The body must be kept in sync with the package kernel if the algorithm
    changes. The correctness check in this benchmark will catch any numerical drift
    immediately because it compares against the parallel kernel for bit equality.
