...
class TestAeroelasticWingMovementSecondDerivativeValidation(unittest.TestCase):
    ...
    def test_non_callable_element_at_index_1_raises_type_error(self):
        """Test that a non-callable, non-None element at index 1 raises TypeError."""
        with self.assertRaises(TypeError):
            self._make_wing_movement(
                spacingAnglesSecondDerivative_Gs_to_Wn_ixyz=[None, "bad", None]
            )

    def test_non_callable_element_at_index_2_raises_type_error(self):
        """Test that a non-callable, non-None element at index 2 raises TypeError."""
        with self.assertRaises(TypeError):
            self._make_wing_movement(
                spacingAnglesSecondDerivative_Gs_to_Wn_ixyz=[None, None, 3.14]
            )
    ...
    def test_valid_tuple_of_none_values_accepted(self):
        """Test that a valid 3-element tuple of None values is accepted and stored."""
        wing_movement = self._make_wing_movement(
            spacingAnglesSecondDerivative_Gs_to_Wn_ixyz=(None, None, None)
        )
    ...
    def test_derivatives_stored_as_tuple_when_list_provided(self):
        """Test that the stored value is always a tuple even when a list is passed."""
        wing_movement = self._make_wing_movement(
            spacingAnglesSecondDerivative_Gs_to_Wn_ixyz=[None, None, None]
        )

        self.assertIsInstance(
            wing_movement.spacingAnglesSecondDerivative_Gs_to_Wn_ixyz, tuple
        )
