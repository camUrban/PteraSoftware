...
class TestLineVortex(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.line_vortex_fixture = tests.unit.fixtures.vortex_fixtures.make_line_vortex_fixture()
    ...
    def test_class(self):
        ...
        self.assertTrue(np.allclose(self.line_vortex_fixture.origin, self.origin_fixture))
        self.assertTrue(np.allclose(self.line_vortex_fixture.termination, self.termination_fixture))
        self.assertEqual(self.line_vortex_fixture.strength, self.strength_fixture)

        self.assertTrue(np.allclose(self.line_vortex_fixture.center, np.array([0.5, 0.5, 0.5])))
        self.assertTrue(np.allclose(self.line_vortex_fixture.vector, np.array([1, 1, 1])))
    ...
    def test_calculate_normalized_induced_velocity(self):
        ...
        self.assertTrue(
            np.allclose(self.line_vortex_fixture.calculate_normalized_induced_velocity(point=point_fixture), 0)
        )
        ...
        point_fixture = self.line_vortex_fixture.center + self.line_vortex_fixture.vector
        self.assertTrue(
            np.allclose(self.line_vortex_fixture.calculate_normalized_induced_velocity(point=point_fixture), 0)
        )
        ...
        r_1_cross_r_2_absolute_magnitude_fixture = (r_1_cross_r_2_fixture[0] ** 2
        ...
        normalized_induced_velocity = (
                ...
                * (r_0_dot_r_1_fixture / r_1_length_fixture - r_0_dot_r_2_fixture / r_2_length_fixture)
        )

        self.assertTrue(
            np.allclose(
                self.line_vortex_fixture.calculate_normalized_induced_velocity(point=point_fixture),
                normalized_induced_velocity
            )
        )
    ...
    def test_calculate_induced_velocity(self):
        ...
        point_fixture = self.line_vortex_fixture.origin
        self.assertTrue(np.allclose(self.line_vortex_fixture.calculate_induced_velocity(point=point_fixture), 0))
        ...
        self.assertTrue(
            np.allclose(self.line_vortex_fixture.calculate_induced_velocity(point=point_fixture), induced_velocity)
        )
